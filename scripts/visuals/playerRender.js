...
export function updHUD(fps, trackLen) {
  ...
  if (keysEl) {
    if (P.secretUnlocked) {
      keysEl.textContent = 'SECRET';
      keysEl.style.color = '#ffd54a';
    } else {
      const need = C.REVERSE_SECRET_DISTANCE || 900;
      const got = Math.min(need, P.reverseDistance || 0);
      keysEl.textContent = `${Math.floor((got / need) * 100)}%`;
      keysEl.style.color = got >= need ? '#9ee36b' : '';
    }
  }

  const trackEl = document.getElementById('h-track');
  if (trackEl) {
    trackEl.textContent = P.secretUnlocked ? 'SECRET ROAD' : 'FAKE ROAD';
    trackEl.style.color = P.secretUnlocked ? '#ffd54a' : '';
  }
}
